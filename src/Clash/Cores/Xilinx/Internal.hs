...
defIpConfig ::
  ...
  IpConfig
defIpConfig name_ version_ moduleName_ =
  IpConfig
    { name = name_
    , version = version_
...
renderTcl = \case
  ...
  ps ->
    let ms = zipWith renderMulti [1 :: Int ..] ps
     in rootSnippet
          <> line
          <> indent 2 "variable scriptPurpose multipleScripts"
          <> line
          <> rbrace
          <> line
          <> vsep ms
 where
  ...
  renderOne (IpConfigPurpose IpConfig{..}) =
   ...
   where
    ...
    renderedProperties =
      "-dict [list "
        <> align (vsep ("\\" : map prop properties))
        <> line
        <> "      ] [get_ips $ipName0]"
  ...
  renderOne (ReadXdcPurpose ReadXdc{..}) = vsep $ catMaybes vars
   where
    ...
    vars :: [Maybe Doc]
    vars =
      [ Just $ var @Text "scriptPurpose" "readXdc"
  ...
  renderMulti n p =
    line
      <> [di|namespace eval ${tclIface}::multipleScripts::script#{n} {|]
      <> line
      <> indent 2 (renderOne p)
      <> line
      <> rbrace
