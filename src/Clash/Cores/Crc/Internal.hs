...
data CrcParams (crcWidth :: Nat) = CrcParams
...
-}
data SoftwareCrc (crcWidth :: Nat) (dataWidth :: Nat) = SoftwareCrc
...
rawResidue crc nExtra = rawResi
 where
  ...
  bitsToFeed =
    fmap (v2bv . reverse . singleton)
      $ V.toList crcVal
      L.++ L.replicate nExtra 0
...
{- | The @F@ and @G@ Matrices for parallel CRC computation, treating
  ...
  '_crcXorOutput'.
-}
data FGMatrices (crcWidth :: Nat) (dataWidth :: Nat) = FGMatrices
...
deriving instance
  (KnownNat crcWidth, KnownNat dataWidth, KnownNat nLanes) =>
...
data CrcHardwareParams (crcWidth :: Nat) (dataWidth :: Nat) (nLanes :: Nat) = CrcHardwareParams
...
class
  ...
  HardwareCrc (crc :: Type) (dataWidth :: Nat) (nLanes :: Nat)
  where
  crcHardwareParams ::
    -- | Which CRC
    crc ->
...
mkCrcLaneParams crc dataWidth@SNat nLanes@SNat =
  case compareSNat2 d1 nLanes of
    ...
    _ ->
      clashCompileError
        "mkCrcLaneParams: Absurd, Report this to the Clash compiler team: https://github.com/clash-lang/clash-compiler/issues"
...
getFGMatrix (CrcLane1 fg) lane =
  case compareSNat2 d1 lane of
    ...
    _ ->
      clashCompileError
        "getFGMatrix, CrcLane1: Absurd, Report this to the Clash compiler team: https://github.com/clash-lang/clash-compiler/issues"
getFGMatrix (CrcLaneN lanes fg next) lane =
  case compareSNat2 lane lanes of
    ...
    _ ->
      clashCompileError
        "getFGMatrix, CrcLaneN: Absurd, Report this to the Clash compiler team: https://github.com/clash-lang/clash-compiler/issues"
...
typeRepToTHType _ =
  error
    "typeRepToTHType: Absurd, Report this to the Clash compiler team: https://github.com/clash-lang/clash-compiler/issues"
...
deriveHardwareCrc crc dataWidth@SNat nLanes@SNat = do
  ...
  [d|
    instance HardwareCrc $crcTy $dataWidthTy $nLanesTy where
...
laneStep (CrcHardwareParams SNat nLanes@SNat (CrcParams SNat _ _ _ _ _) laneParams _) lanePrev@SNat crc input =
  let lane = addSNat lanePrev d1
   in case compareSNat lane nLanes of
        SNatLE -> matVecMul (getFGMatrix laneParams lane) (crc ++# (pack $ takeLe lane input))
        _ ->
          clashCompileError
            "laneStep: Absurd, Report this to the Clash compiler team: https://github.com/clash-lang/clash-compiler/issues"
