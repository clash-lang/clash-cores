...
toVecN a = out
 where
  ...
  out = case List.findIndex isJust raw0 of
    Just i ->
      List.take i raw1
        List.++ [addReset True $ raw0 List.!! i]
        List.++ List.drop (i + 1) raw1
    Nothing -> raw1
...
dataWidthCfgtoSomeNat ::
  forall crcWidth n. (KnownNat crcWidth) => (KnownNat n) => Index n -> SomeNat
...
crcEngineResetProp crc checkVal inp = hwCrc QC.=== checkVal
 where
  ...
  someInp =
    fmap toInp $ List.zip [(0 :: Int) ..] $ inp
      List.++ (getCheckInput SNat dw _crcReflectInput)
...
crcEngineEqSoftware crc nLanesI dataWidthCfg inpI =
  go
    (dataWidthCfgtoSomeNat @(CrcWidth crc) dataWidthCfg)
    (fromJust $ someNatVal (1 + fromIntegral nLanesI))
 where
 ...
 where
  ...
  prop nLanes@SNat dataWidth@SNat = hwOk QC.=== True
   where
    ...
    swCrcVec =
      applyWhen _crcReflectInput reverse
        $ bitCoerce
    ...
    hwParams = mkCrcHardwareParams crc dataWidth nLanes
    crcValidator' =
      exposeClockResetEnable crcValidatorFromParams systemClockGen resetGen enableGen hwParams
