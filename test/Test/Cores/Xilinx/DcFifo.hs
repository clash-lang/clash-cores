...
prop_fifoOrder d pr pw feed drain = property $ do
  (xs, wrIn, rdStalls) <-
    forAll $ genScenario (Gen.int (Range.linear 30 40)) (Gen.int (Range.linear 10 15))
...
prop_noException d pr pw feed drain = property $ do
  (_, wrIn, rdStalls) <-
    forAll $ genScenario (Gen.int (Range.linear 7 12)) (Gen.int (Range.linear 7 8))
  let res = throughFifo d pr pw feed drain wrIn rdStalls
  H.annotate (show res)
...
testOverflow = testCase "Overflows appropriately" $ do
  ...
  assertBool
    "Doesn't overflow if we write exactly 15 values"
...
takeState ::
  ...
  (Empty, DataCount depth, BitVector n) ->
  ((Bool, [Maybe Int]), (Maybe (BitVector n), Bool))
takeState (readLastCycle, Just n : stalls) (_, _, d)
  | n > 0 =
...
takeClumsy ::
  ...
  (Empty, DataCount depth, BitVector n) ->
  ((Bool, [Maybe Int]), (Maybe (BitVector n), Bool))
takeClumsy (readLastCycle, Just n : stalls) (_, _, d)
  | n > 0 =
      ((False, Just (n - 1) : stalls), (nextData, False))
 where
  nextData = readLastCycle `orNothing` d
